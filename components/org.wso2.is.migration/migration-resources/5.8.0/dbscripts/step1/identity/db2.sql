...
BEGIN
	IF NOT EXISTS (SELECT * FROM SYSCAT.TABLES where TABNAME = 'IDN_OAUTH2_ACCESS_TOKEN' AND INDEX_TBSPACE = 'TS32K')
	THEN
...
CREATE OR REPLACE PROCEDURE EXECUTE_ADMIN_MOVE_TABLE
BEGIN
	IF NOT EXISTS (SELECT * FROM SYSCAT.TABLES where TABNAME = 'IDN_OAUTH2_AUTHORIZATION_CODE' AND INDEX_TBSPACE = 'TS32K')
	THEN
		CALL SYSPROC.ADMIN_MOVE_TABLE(
		(SELECT TABSCHEMA FROM SYSCAT.TABLES where TABNAME = 'IDN_OAUTH2_AUTHORIZATION_CODE'),
		'IDN_OAUTH2_AUTHORIZATION_CODE',
		(SELECT TBSPACE FROM SYSCAT.TABLES where TABNAME = 'IDN_OAUTH2_AUTHORIZATION_CODE'),
		'TS32K',
		(SELECT TBSPACE FROM SYSCAT.TABLES where TABNAME = 'IDN_OAUTH2_AUTHORIZATION_CODE'),
		'',
		'',
